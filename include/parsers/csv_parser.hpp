...
#define CSV_PARSER_DELETE_BUFFER_PTR(ptr, size) \
if (ptr != nullptr)								\
{												\
...
public :
	...
	bool reset(const char* input_filename);
	/**
	...
	void set_enclosed_char_vector(int a_size, char fields_enclosed_by[]);
	/**
	...
	/**
	 * Defines the vector of fields delimiter used in the text file
	 *
	 * @param[in] fields_terminated_by
	 * @return void
	 */
	void set_field_term_char_vector(int a_size, int fields_terminated_by[]);
	...
	void set_ignore_comments(int a_size, char* a_comments_symbols[5]);
...
protected :
	...
	/** enclosers **/
	char* field_enclose_char_vector;
	int field_enclose_char_vector_length;
	bool enclosed_char_vector_contains(const char& current_char);

	/** delimiters **/
	int* field_term_char_vector;
	int field_term_char_vector_length;

	/** comments **/
	char** m_comments_symbols;
	int m_comments_symbols_length;

	bool check_current_char_in_delimiters(char* current_char);
